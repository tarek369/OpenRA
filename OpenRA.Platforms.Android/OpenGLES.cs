...
namespace OpenRA.Platforms.Android
{
	...
	static class OpenGL
	{
		...
		{
			IntPtr ptr = Egl.eglGetProcAddress(name);

			// On Android, eglGetProcAddress is allowed to return NULL for core (non-extension)
			// entry points on some drivers. For those we fall back to a direct DllImport lookup
			// against libGLESv2.so via the CoreGL resolver.
			if (ptr == IntPtr.Zero)
				ptr = CoreGL.GetProcAddress(name);

			if (ptr == IntPtr.Zero)
				throw new EntryPointNotFoundException($"Failed to load GL function '{name}'.");

			return (T)(object)Marshal.GetDelegateForFunctionPointer(ptr, typeof(T));
		}

		static bool ExtensionSupported(string name)
		{
			// GLES3 exposes extensions via glGetStringi(GL_EXTENSIONS, i) rather than the
			// single glGetString(GL_EXTENSIONS) string used by GLES2/desktop.
			if (ExtensionSet == null)
			{
				var set = new HashSet<string>();
				var count = 0;
				glGetIntegerv(GL_NUM_EXTENSIONS, out count);
				for (var i = 0; i < count; i++)
				{
					var ext = glGetStringi(GL_EXTENSIONS, (uint)i);
		...
		static HashSet<string> ExtensionSet;
